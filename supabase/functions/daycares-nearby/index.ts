...
// The DCF registry licenses school-age aftercare programs hosted at elementary/
// middle/high schools (227 of 1,552 seed rows). villie's audience is 0-12
// months — school-age programs are noise on every source, so drop them.
const SCHOOL_RE = /(middle school|high school|elementary|k-8|k 8|junior high|senior high|charter school)/i;
const isSchoolAge = (name: string) => SCHOOL_RE.test(name);
...
async function fetchMiamiDaycares(lat: number, lng: number, radiusMiles: number): Promise<DaycareResult[]> {
  ...
  const results: DaycareResult[] = ((data ?? []) as any[])
    .filter((d) => !isSchoolAge(String(d.name ?? '')))
    .map((d) => ({
      place_id: d.id,
      name: d.name,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Miami-Dade → DCF registry MERGED with Places discovery. The registry seed
    // misses real centers (e.g. newer ones — "Beehive" in Coconut Grove), so
    // Places fills the gaps; registry rows win on dupes (they carry license #).
    if (inMiamiDade(lat, lng)) {
      const [miami, places] = await Promise.all([
        fetchMiamiDaycares(lat, lng, radiusMiles).catch(() => [] as DaycareResult[]),
        fetchDaycares(lat, lng, radiusMiles).catch(() => [] as DaycareResult[]),
      ]);
      if (miami.length || places.length) {
        const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, '');
        const regNames = miami.map((m) => norm(m.name));
        const extras = places.filter((p) => {
          const pk = norm(p.name);
          // Name-containment dedupe only (min length guards trivial matches) —
          // proximity-based dedupe false-drops distinct centers on dense blocks.
          return !regNames.some((rk) => (rk.length >= 6 && pk.includes(rk)) || (pk.length >= 6 && rk.includes(pk)));
        });
        const merged = [...miami, ...extras].sort((a, b) => a.distance_mi - b.distance_mi);
        return json({ count: merged.length, results: merged, source: 'mdc_dcf+places' });
      }
      // Neither source returned in range → fall through to the plain Places error path.
    }
