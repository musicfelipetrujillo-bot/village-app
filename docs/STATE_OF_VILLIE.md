...
> 🚨 **MIGRATION 132 IS APPLIED ON PROD BUT ITS `.sql` EXISTS ONLY ON `feat/pro-locale-gate`.** `main` runs `130 → 131 → **gap** → 133 → 134 → 135 → 136`. **Verified live 2026-09-08**, not inferred: `GET /rest/v1/pro_launch_targets?select=locales` returns `200 []` (column present), while a bogus column on the same table returns `400 / 42703`. So the column 132 adds is really there. **This is the exact shape that hard-fails `supabase db push` for every session** — a remote-applied version with no local file. Do not run `migration repair --status reverted`; restore the file instead: `git show feat/pro-locale-gate:supabase/migrations/132_pro_launch_locale_scope.sql > supabase/migrations/132_pro_launch_locale_scope.sql`, or merge the branch. ⚠️ The migration *ledger* itself could not be read (anon key can't reach `supabase_migrations`), so "132 is recorded as applied" is inferred from the column, not confirmed.
