...
- Find or create the App ID for bundle `com.thevillage.app`
...
- Identifier: `com.thevillage.app.auth` (this is the Services ID Supabase uses; the suffix is convention but any unique reverse-DNS works)
- Save → click into the created Services ID → enable **Sign In with Apple** → click **Configure**:
  - Primary App ID: `com.thevillage.app`
...
- Check **Sign In with Apple** → click **Configure** → primary App ID `com.thevillage.app` → save
...
- `APPLE_SERVICES_ID` — from 2.2 (e.g. `com.thevillage.app.auth`)
...
- Authorized Client IDs: paste `APPLE_SERVICES_ID` AND the App ID `com.thevillage.app` (comma-separated). Both are needed because the iOS-native Apple Sign-In returns tokens with `aud` claim = the App ID, while the web OAuth flow uses the Services ID.
...
4.1. Open `apps/mobile/app.json` → find the `@react-native-google-signin/google-signin` plugin block → replace `REPLACE_WITH_GOOGLE_IOS_REVERSED_CLIENT_ID` with the actual reversed client ID from step 1.4 (`com.googleusercontent.apps.1234...`). This is the URL scheme iOS uses to handshake back into the app after Google's auth sheet completes.
...
client_id = 'com.thevillage.app.auth'   # Services ID
