...
- **Stale Info.plist permission strings — DELETE before submit** — audit done 2026-05-28 found 3 declared permissions with zero runtime API calls (Apple Review rejection risk). Remove from `apps/mobile/app.json` → `ios.infoPlist`: `NSContactsUsageDescription`, `NSMicrophoneUsageDescription`, `NSLocationAlwaysUsageDescription`. Keep `NSLocationWhenInUseUsageDescription`. Full rationale in `docs/APP_PRIVACY_QUESTIONNAIRE.md` Step 5.
