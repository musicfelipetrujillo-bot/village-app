...
| `RENDER_PROXY_KEY` | events-harvest (Villie Plans JS-render fallback) | **yes, to source anything JS-rendered** | jina.ai → API key (free tier) | `events-harvest` does a plain `fetch()` and executes no JS, so hospital SPAs, the county-library calendar and Mindbody/Momence studio widgets extract **zero** events. The fallback routes those through `r.jina.ai`, which **returns 403 from Supabase edge IPs without a key**. Unset → fallback is wired but inert and only server-rendered sources work (PSI, The Underline). Don't register JS-only sources until this is set: zero-yield now counts as a failure, so they'd auto-deactivate in 3 days. Verify with `{"probe":{"url":"…"}}` → `needs_render:true` means the proxy did the work. |
| `GOOGLE_MAPS_API_KEY` | events-geocode **+ geocode-zip (ZIP→centroid location fallback)** | yes | Google Cloud console → Geocoding API | one key shared. Without it `geocode-zip` returns 503 and "near me" falls back to the launch-market default instead of the mother's own ZIP. |
...
**✅ BACKEND FULLY DEPLOYED (2026-07-29).** Migrations `104` + `105` recorded on hosted; all three edge functions ACTIVE (`trending-ingest` `--no-verify-jwt` / shared-secret, `trending-compliance-pass` + `trending-publish-notify` verify_jwt=true / service-role-locked); `TRENDING_INGEST_SECRET` set in Edge Function Secrets **and** mirrored to gitignored `supabase/.env.local` (where the Step-B scheduled task reads it); `ANTHROPIC_API_KEY` + `SUPABASE_URL/SERVICE_ROLE_KEY` set. **The clinical reviewer is `fele_trujillo@hotmail.com`** (the founder's primary Apple login) — migrations 106–108 moved `is_clinical_reviewer` + `is_event_reviewer` there and revoked both from the secondary account `felitrujillo95@hotmail.com`. Verified against prod 2026-07-30. First real issue already ingested (`c3cc6efb…`, 4 medical items sitting `in_review`).
...
**End-to-end:** the two scheduled tasks run weekly (discovery Fri → sourcing+ingest 30 min later). Medical-claim items land `in_review` → appear in ClinicalReviewScreen (reviewer accounts) → approve → the last approve auto-publishes the issue → Home shows the "the buzz is here" card. Non-medical items auto-clear via `trending-compliance-pass`. **To finish the current dry run:** sign in as **`fele_trujillo@hotmail.com`** (primary Apple login — the reviewer account since migrations 106–108; the old `felitrujillo95@hotmail.com` no longer has the flag), open ClinicalReviewScreen, approve all 4 items on issue `c3cc6efb…` → it auto-publishes.
